...
| Latest full publication gate | ready | `scripts/prepublish-check.sh` passed locally on code commit `4ae0412`, including `scripts/release-candidate.sh`, package/install verification, crate/tag availability checks, and `cargo publish --dry-run --locked`. GitHub Actions default `release-gate` passed on code commit `4ae0412`; later documentation-only commits may reuse this evidence. |
