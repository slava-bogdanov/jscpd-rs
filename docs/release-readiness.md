...
| Latest full publication gate | ready | `scripts/release-candidate.sh` passed locally on code commit `d1c759c`, and GitHub Actions `workflow_dispatch release_candidate=true public_runs=3` passed on code commit `d1c759c`; both include clippy, the default gate, the full coverage matrix, and the public benchmark/coverage suite. |
