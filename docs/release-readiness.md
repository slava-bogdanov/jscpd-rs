...
| Latest full publication gate | ready | `scripts/release-candidate.sh` passed locally on code commit `04563ac`, and GitHub Actions `workflow_dispatch release_candidate=true public_runs=3` passed on code commit `39a6a70`; both include clippy, the default gate, the full coverage matrix, and the public benchmark/coverage suite. |
