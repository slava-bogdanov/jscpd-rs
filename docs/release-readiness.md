...
| Upstream JavaScript API parity | follow-up | Native Rust helpers cover the practical app/tokenizer concepts, including an embeddable argv runner and tokenizer map generation; exact JS package export shape is not implemented in the Rust crate. See `docs/api-parity.md`. |
...
| Latest full publication gate | ready | `scripts/prepublish-check.sh` passed locally on code commit `bac2203`, including `scripts/release-candidate.sh`, package/install verification, crate/tag availability checks, and `cargo publish --dry-run --locked`. GitHub Actions default `release-gate` passed on code commit `bac2203`; later documentation-only commits may reuse this evidence. |
